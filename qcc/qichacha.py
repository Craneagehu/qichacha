...
def qcc(company):
    ...
    #获取企业详情链接
    link = e.xpath('//*[@id="search-result"]/tr[1]/td[3]/a/@href')
    if link:
        detail_link = urljoin(res.url,link[0])
...
if __name__ == '__main__':
    ...
    df = pd.read_excel('C:/Users/MSI1/Desktop/公司信息表.xlsx',nrows=50) #前50行
    #转换为python格式的列表
    data =df.iloc[:,[3]].values.tolist()
    data = [each[0] for each in data]
    for company in data:
        items = qcc(company)
